...
@author: Camilo Montenegro & Valentina Chacon
"""
...
def read_settings(settings, config_path):
    """Catch parameters fron console or code defined"""
    config = cp.ConfigParser(interpolation=None)
    print("reading settings at path: ")
    print(config_path)
...
best_param = 1000000000000000000

def objective(params, config_path):
    global best_param
    settings = read_settings(params, config_path)
    ...
    for f in settings['flag'].split(","):
        ...
        if f == 1:
            ...
            if optimization:
                ...
                if optimal_val<best_param:
                    best_param = optimal_val
                    print('--- Global optimal', settings['criteria'], 'for', settings['objective'], 'with happy path:', settings['happy_path'],'is:',
                          str(optimal_val), unit, 'found in configuration k =', str(optimal_k), 'and in iteration:', str(optimal_key), '---')
                print("~~~~~~~~~~~~~~~~~~~~~~el valor mas optimo es: ", best_param)
            ...
            if optimization:
                ...
                if optimal_val<best_param:
                    best_param = optimal_val
                    print('--- Global optimal', settings['criteria'], 'for', settings['objective'], 'with happy path:', settings['happy_path'], 'is:',
                    str(optimal_val), unit, 'found in configuration with similitude percentage =', str(optimal_perc), 'and in iteration:', str(optimal_key), '---')
                print("~~~~~~~~~~~~~~~~~~~~~~el valor mas optimo es: ", best_param)
...
#TODO: AS AN INPUT ASK FOR THE OPTIMIZATION FUNCTION, CALL OBJECTIVE FOR ALL 4 CONFIGURATION FILES(BY SENDING A DIFFERENT PATH TO OBJECTIVE)
def main(argv):
    space = {
        'epsilon': 1,
        'eta': 1,
    ...
    print("select an objective function: cost_total, flow_time, time_workload or waiting")
    objective_function = input()
    print("This is the function you selected: ")
    print(objective_function)
    for i in range(1,5):
        file_name = 'config' + str(i) + '.txt'
        config_path = './configurations/'+ objective_function + '/' + file_name
        objective(space, config_path)
